...
## Backup Strategie CronJob
Shell script was verbunden is mit einem Cronjob
...
PASSWORD=""
DB=""
BACKUP_DIR="/var/www/petrilog.com/backup"
DATE=$(date +%Y-%m-%d)

mkdir -p "$BACKUP_DIR"

# ==============================
# Datenbankstruktur sichern (ohne bestimmte Tabellen)
# ==============================
mysqldump -u $USER -p$PASSWORD --no-data --add-drop-table \
  --ignore-table=${DB}.jobs \
  --ignore-table=${DB}.failed_jobs \
  $DB > "$BACKUP_DIR/structure_$DATE.sql"

# ==============================
# Daten sichern (ohne Cache, Sessions, Pulse, etc.)
# ==============================
mysqldump -u $USER -p$PASSWORD --no-create-info \
  ...
  $DB > "$BACKUP_DIR/data_$DATE.sql"

# ==============================
# Media-Dateien sichern (nur Inhalt, ohne lange Pfade)
# ==============================
...
if [ -d "$MEDIA_SRC" ]; then
    cd "$MEDIA_SRC"
    zip -r "$MEDIA_ZIP" ./*
    echo "Media backup created: $MEDIA_ZIP"
else
    echo "Media source folder not found: $MEDIA_SRC"
fi

# ==============================
# Alte Backups rotieren (nur die neuesten 4 behalten)
# ==============================
cd "$BACKUP_DIR"

# Struktur + Daten (SQL)
ls -1t structure_*.sql data_*.sql | tail -n +5 | xargs -r rm --

echo "Backup complete. Old backups rotated.