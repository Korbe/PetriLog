    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-10">
      <div v-for="state in states" :key="state.id">
        <Link :href="`/states/${state.id}`">
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow hover:shadow-lg transition-shadow duration-200 overflow-hidden">
            
            <!-- Bild -->
            <div class="relative w-full h-44">
              <img 
                :src="state.media[0]?.url ?? '/placeholder.png'" 
                :alt="state.name"
